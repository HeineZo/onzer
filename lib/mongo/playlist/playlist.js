...
export async function removeMusicFromPlaylist(id, musicIds) {
  ...
  try {
    ...
    success = await playlist.updateOne(
      ...
      {
        $pullAll: {
          musiques: musicIds.map((musicId) => new ObjectId(musicId)),
        },
      }
    )
 ...
 * @param idMusic Identifiant de la musique
 */
export async function addMusicToPlaylist(idPlaylist, idMusic) {
  let success = false
  let message = "Impossible d'ajouter la musique à la playlist"
  try {
    if (!playlist) {
      await init()
    }
    ...
    if (success) {
      success = success.acknowledged
      message = `La musique a été ajouté à la playlist`
    }
  } catch (e) {
    console.log(e)
  }
...
export async function editPlaylist(data) {
  let success = false
  let message = "Impossible de modifier la playlist"
  try {
    if (!playlist) {
      await init()
    }
 ...
 * @param id Identifiant permettant d'identifier la playlist à supprimer
...
export async function deletePlaylist(id) {
  let success = false
  let message = "Impossible de supprimer la playlist"
  try {
    if (!playlist) {
      await init()
    }
    success = await playlist.deleteOne({ _id: new ObjectId(id) })
    success = success.acknowledged
    if (success) {
